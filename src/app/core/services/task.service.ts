...
export class TaskService {
  private tasks: Task[] = [
    ...
    { id: '5', title: 'Database Optimization', description: 'Optimize slow performing queries in the production database.', assignedTo: 'John Doe', priority: 'High', status: 'In Progress', startDate: new Date('2026-04-12'), dueDate: new Date('2026-04-25'), createdAt: new Date('2026-04-05') },
    { id: '6', title: 'Security Audit', description: 'Perform a comprehensive security scan of the entire infrastructure.', assignedTo: 'Alice Wong', priority: 'High', status: 'In Progress', startDate: new Date('2026-05-01'), dueDate: new Date('2026-05-15'), createdAt: new Date('2026-04-15') },
  ...
  getTasks(): Observable<Task[]> {
    return this.tasksSubject.asObservable().pipe(
      map(tasks => [...tasks].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()))
    );
  }
